 ...
 */
function wp_pelagios_update_options( $options )
{    
    if ( empty( $_POST ) )
    	return false;    
    ...
    foreach ( $options as $value ) {
		if ( isset( $value['type'] ) && $value['type'] == 'checkbox' ) {            
    		...
    		if ( isset( $value['id'] ) && isset( $_POST[$value['id']] ) ) {
            	update_option($value['id'], wp_pelagios_clean($_POST[$value['id']]));
 ...
 * Admin fields
 * 
 * Loops though the wp_pelagios options array and outputs each field.
 */
function wp_pelagios_admin_fields( $options )
{
	global $wp_pelagios;
    ...
    foreach ( $options as $value )
    {
        ...
        switch($value['type']) {
            case 'title':
            	...
            	if ( isset($value['id'] ) && $value['id'] ) do_action( 'wp_pelagios_settings_' . sanitize_title($value['id'] ) );
            break;
            case 'sectionend':
            	if ( isset($value['id'] ) && $value['id'] ) do_action( 'wp_pelagios_settings_' . sanitize_title( $value['id'] ) . '_end' );
            	echo '</table>';
            	if ( isset($value['id'] ) && $value['id'] ) do_action( 'wp_pelagios_settings_' . sanitize_title( $value['id'] ) . '_after' );
            break;
            case 'text':
            	?><tr valign="top">
            ...
            break;
            default:
            	do_action( 'wp_pelagios_admin_field_'.$value['type'], $value );
            break;
    	}
	}
}
